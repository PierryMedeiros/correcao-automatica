...
- [ ] **Decidir de onde sai o compose do aluno para o override ser gerado** — costura aberta pela F2 em 2026-08-07, e ela precisa ser resolvida **antes** de montar o comando canônico, que já referencia o arquivo do override pelo caminho. A F2 entregou o gerador puro (`gerarOverrideDoCompose`, com testes) e o job dir escreve `compose-aluno.yaml` + `compose.override.yaml` **quando o host já conhece o compose** — que é o caso do job fake, e não é o caso de uma entrega real: o compose do aluno só existe depois do clone, e quem clona é o entrypoint (§9.2 passo 4, Apêndice B (06/08) item 5). Três saídas, todas com custo: **(a)** o entrypoint chama o gerador depois do clone — exige o gerador dentro da imagem, e duas cópias do mesmo gerador é o que não pode acontecer; **(b)** o entrypoint sinaliza o fim do clone (o `clone.json` já é escrito ali) e espera o override aparecer, com o controller gerando do host a partir de `<job_dir>/repo/...` — handshake novo, muda §8/§9.2 e por isso passa pelo plano primeiro; **(c)** o prompt entrega o comando sem override e manda o agente gerá-lo — contraria o §8 ("o prompt não pede ao agente que invente `-p` nem gere override"). A decisão também precisa dizer **qual** arquivo é o compose do aluno (`compose.yaml`, `docker-compose.yml`, em subpasta) e o que fazer quando não há nenhum
